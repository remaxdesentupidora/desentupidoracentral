...
export interface RecentArticlesBlockData {
  type: 'recentArticles';
  heading?: string;
  count: number;
}
...
export function createEmptyBlock(type: PageBlockType): PageBlock {
  switch (type) {
    ...
    case 'recentArticles':
      return { type: 'recentArticles', count: 3 };
  }
}
...
function parseArticleCount(value: unknown): number {
...
export function parsePageBlocks(value: unknown): PageBlock[] {
  ...
  for (const raw of value) {
    ...
    if (type === 'text') {
      ...
      blocks.push({
        type: 'text',
        layout: layout === 'centered' ? 'centered' : layout === 'simple' ? 'simple' : undefined,
    ...
    if (type === 'recentArticles') {
      blocks.push({
        type: 'recentArticles',
        heading: asString(raw.heading) || undefined,
        count: parseArticleCount(raw.count),
      });
    }
  }
...
export function serializePageBlock(block: PageBlock): Record<string, unknown> {
  switch (block.type) {
    ...
    case 'recentArticles': {
      const data: Record<string, unknown> = {
        type: 'recentArticles',
        count: parseArticleCount(block.count),
      };
