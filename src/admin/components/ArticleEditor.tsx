...
function featuredImageFromData(data: Record<string, unknown>): { src: string; alt: string; credit: string } {
  ...
  if (featured && typeof featured === 'object' && !Array.isArray(featured)) {
    ...
    return { src: asString(image.src), alt: asString(image.alt), credit: asString(image.credit) };
  }
...
function toFrontmatterData(form: FormState, mode: 'create' | 'edit'): Record<string, unknown> {
  ...
  const data: Record<string, unknown> = {
    ...
    featuredImage: {
      ...
      ...(form.featuredImageCredit.trim()
        ? { credit: form.featuredImageCredit.trim() }
        : {}),
    },
