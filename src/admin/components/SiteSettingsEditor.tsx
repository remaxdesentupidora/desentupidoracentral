...
interface AreaServedRow {
  uid: string;
  value: string;
}

type SiteTheme = 'modern' | 'classic';
...
function sitemapIndexUrl(siteUrl: string): string {
  const trimmed = siteUrl.trim().replace(/\/+$/, '');
  return trimmed ? `${trimmed}/sitemap-index.xml` : '';
}
...
function parseSettings(raw: string): FormState {
  ...
  return {
    ...
    telephone: asString(organization.telephone),
    areaServed,
    whatsappNumber: asString(organization.whatsappNumber),
    sameAs,
    theme: entry.theme === 'classic' ? 'classic' : 'modern',
  };
}
...
function toSettingsPayload(form: FormState): Record<string, unknown> {
  ...
  if (form.legalName.trim()) {
    ...
    organization.name = form.legalName.trim();
  }
  ...
  organization.telephone = form.telephone.trim();
  if (form.whatsappNumber.trim()) {
    organization.whatsappNumber = form.whatsappNumber.trim();
  }
  organization.areaServed = form.areaServed.map((row) => row.value.trim()).filter(Boolean);
...
export default function SiteSettingsEditor({ token }: Props) {
  ...
  const sitemapLink = sitemapIndexUrl(form.siteUrl);

  async function copySitemapLink() {
    if (!sitemapLink) {
      return;
    }

    try {
      await navigator.clipboard.writeText(sitemapLink);
      setSitemapCopied(true);
      window.setTimeout(() => setSitemapCopied(false), 2000);
    } catch {
      setFormError('Não foi possível copiar o link do sitemap.');
    }
  }
    ...
    <div class="admin-settings">
      ...
      <div class="admin-settings__layout">
        ...
        <form class="admin-card admin-form" onSubmit={handleSubmit} noValidate>
          ...
          <div class="admin-field">
            <label htmlFor="settings-sitemap">Link do Sitemap</label>
            <div class="admin-copy-row">
              <input
                ...
                value={sitemapLink}
                readOnly
                disabled={saving || !sitemapLink}
                spellCheck={false}
                aria-describedby="settings-sitemap-hint"
              />
              <button
                type="button"
                class="admin-button"
                disabled={saving || !sitemapLink}
                onClick={() => void copySitemapLink()}
              >
                {sitemapCopied ? 'Copiado!' : 'Copiar link'}
              </button>
            </div>
            <p class="admin-field__hint" id="settings-sitemap-hint">
              Envie este link ao cadastrar seu site no Google Search Console.
            </p>
          </div>
          ...
          <fieldset class="admin-fieldset">
            ...
            <div class="admin-weight-list" role="radiogroup" aria-label="Tema visual">
              <label class="admin-checkbox">
                <input
                  type="radio"
                  name="settings-theme"
                  value="modern"
                  checked={form.theme === 'modern'}
                  disabled={saving}
              ...
              <label class="admin-checkbox">
                <input
                  ...
                  name="settings-theme"
                  value="classic"
                  checked={form.theme === 'classic'}
                  disabled={saving}
                  onChange={() => update('theme', 'classic')}
                />
                Clássico
              </label>
            </div>
          ...
          <fieldset class="admin-fieldset">
            ...
            <div class="admin-field">
              <label htmlFor="settings-org-telephone">Telefone (opcional)</label>
              <input
                id="settings-org-telephone"
                type="tel"
                value={form.telephone}
                onInput={(event) => update('telephone', (event.target as HTMLInputElement).value)}
                disabled={saving}
                spellCheck={false}
            ...
            <div class="admin-field">
              <span>Áreas atendidas (opcional)</span>
              {form.areaServed.length === 0 ? (
                <p class="admin-empty">Nenhuma área ainda.</p>
              ) : (
                <ul class="admin-repeat-list">
                  {form.areaServed.map((row) => (
                    <li key={row.uid} class="admin-repeat-row">
                      <div class="admin-field">
                        <label htmlFor={`area-served-${row.uid}`}>Localidade</label>
                        <input
                          id={`area-served-${row.uid}`}
                          type="text"
                          value={row.value}
                          onInput={(event) =>
                            update(
                              'areaServed',
                              form.areaServed.map((item) =>
                                item.uid === row.uid
                      ...
                      <button
                        ...
                        onClick={() =>
                          update(
                            'areaServed',
                            form.areaServed.filter((item) => item.uid !== row.uid),
                          )
                        }
                      >
                        Remover
              ...
              <button
                ...
                class="admin-button admin-button--ghost"
                disabled={saving}
                onClick={() => update('areaServed', [...form.areaServed, { uid: newUid(), value: '' }])}
              >
                Adicionar área
              </button>
            </div>
            <div class="admin-field">
              <label htmlFor="settings-org-whatsapp">WhatsApp (opcional)</label>
              <input
                id="settings-org-whatsapp"
                type="tel"
                value={form.whatsappNumber}
                onInput={(event) => update('whatsappNumber', (event.target as HTMLInputElement).value)}
                disabled={saving}
                spellCheck={false}
                placeholder="5541999999999"
                aria-describedby="settings-org-whatsapp-hint"
              />
              <p class="admin-field__hint" id="settings-org-whatsapp-hint">
                Formato esperado: 55DDNNNNNNNNN (código do país + DDD + número, só dígitos).
              </p>
            </div>
            <div class="admin-field">
              <span>sameAs</span>
