...
import RecentArticlesBlockEditor from './blocks/RecentArticlesBlockEditor';
...
const BLOCK_OPTIONS: Array<{ type: PageBlockType; label: string }> = [
  ...
  { type: 'cta', label: 'CTA' },
  { type: 'recentArticles', label: 'Artigos Recentes' },
];
...
function validatePage(form: FormState): string[] {
  ...
  form.blocks.forEach((block, index) => {
    ...
    switch (block.data.type) {
      ...
      case 'recentArticles':
        if (!Number.isInteger(block.data.count) || block.data.count < 1 || block.data.count > 12) {
          errors.push(`${label}: informe uma quantidade entre 1 e 12.`);
        }
        break;
    }
  });
...
function fieldErrorsFromMessages(form: FormState): Record<string, string> {
  ...
  form.blocks.forEach((block) => {
    switch (block.data.type) {
      ...
      case 'recentArticles':
        if (!Number.isInteger(block.data.count) || block.data.count < 1 || block.data.count > 12) {
          errors[`${block.uid}.count`] = 'Informe uma quantidade entre 1 e 12.';
        }
        break;
    }
  });
      ...
      <form class="admin-card admin-form" onSubmit={handleSubmit} noValidate>
        ...
        <section class="admin-blocks">
          ...
          {form.blocks.length === 0 ? (
            <p class="admin-empty">Nenhum bloco ainda. Adicione Hero, Texto, Galeria, CTA ou Artigos Recentes.</p>
          ) : (
